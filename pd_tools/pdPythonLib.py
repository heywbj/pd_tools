...
def InterpretString3(msgstr):
    ...
    currIdx = find(msgstr,'[')
    if (currIdx!=-1): #might be an array or a 2d array
        arrStr = re.split("\s*",msgstr)
        del arrStr[0] #if it is an array, first element is ''
        arrStrlen = len(arrStr)
        del arrStr[arrStrlen-1] #last element is the \000 character
        arrList = []

        #test to see if it is an array or a 2d array
        #1d array format of arrStr[0] MUST BE:
